...
export function WatchOrb({
  ...
  const small = compact || docked;
  return (
    <button
      type="button"
      aria-label="What should I watch?"
      aria-busy={loading}
      disabled={loading}
      onClick={onClick}
      className={cn(
        "group pointer-events-auto flex flex-col items-center bg-transparent p-0 outline-none focus-visible:ring-2 focus-visible:ring-violet-300/50",
        docked ? "gap-0" : small ? "gap-4" : "gap-14",
        loading ? "cursor-wait" : "cursor-pointer"
      )}
    >
      <style>{WATCH_ORB_CSS}</style>
      {docked ? (
        <span className="sr-only">What should I watch?</span>
      ) : (
        <span
          className={cn(
      ...
      >
        <span
          ...
          className={cn(
            ...
            docked ? "size-16 blur-md" : small ? "size-20 blur-lg" : "size-52 blur-3xl"
          )}
        />
        <span
          aria-hidden
          className={cn(
            ...
            docked ? "size-11 blur-sm" : small ? "size-14 blur-md" : "size-40 blur-2xl",
        ...
        {loading ? (
          <span
            ...
            className={cn(
              ...
              docked ? "size-14" : small ? "size-16" : "size-[9.5rem]"
            )}
          />
        ) : null}
        <span
          className={cn(
            ...
            docked && "ring-1 ring-white/20",
            docked
              ? "size-11 shadow-[0_0_22px_rgba(124,58,237,0.55)]"
              : small
        ...
        >
          <svg
            ...
            className={cn(
              ...
              docked ? "size-5" : small ? "size-7" : "size-14"
            )}
          >
